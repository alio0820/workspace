...
public class MyFile {
	...
	public static ArrayList<FileBean> getFileList(String path){
		ArrayList<FileBean> list = new ArrayList<FileBean>();
		String currentPath = path;
		File file = new File(currentPath);
		if(!file.exists()){
			ILog.jspInfo("文件路径：{"+currentPath+"}不存在");
			return list;
		}
		...
		for(int i=0;i<filelist.length;i++){
			...
			if(fName.equals(".svn")||fName.equals(".DS_Store"))
				continue;
			...
			fileBean.setCurrentPath(fName);
			fileBean.setName(fName);
			list.add(fileBean);
		}
		return list;
	}
