	...
	return{
		...
		initTabs : function(){
			...
			$('ul#myTab a').eq(0).bind("click",function(){
				$('#info').removeClass('tabs-2');
				$('#topicDesignMain').hide();
				if($('#topicImport').find('.box-content').length!=0)
					$('#topicImport').show();
			});
			$('ul#myTab a').eq(1).bind("click",function(){
				$('#info').addClass('tabs-2');
				$('#topicImport').hide();
				if($('#topicDesignMain').find('.box-content').length!=0)
					$('#topicDesignMain').show();
			});
			$('ul#myTab a').slice(2).bind("click",function(){
				$('#topicImport').hide();
				$('#topicDesignMain').hide();
			});
			contentApp.initQueryLabel();
		},
		initQueryLabel : function(){
			$('div.queryUnit label').click(function(){
				var queryStyle = $(document.getElementById("is-ajax2").parentNode).hasClass('checked')?2:1;
				if(queryStyle==1){
					$('div.queryUnit').find('i').addClass('tabs-hide');
					$(this).find('i').removeClass('tabs-hide');
				}else{
					var i = $(this).find('i');
					if($(i).hasClass('tabs-hide'))
						$(i).removeClass("tabs-hide");
					else
						$(i).addClass("tabs-hide");
				}
			});
		},
