 ...
 * 查看试卷
 */
function as_view_paper(obj){
	var tr = $(obj).parent().parent();
	var paperId = $(tr).attr('id');
	var paperName = $(tr).find('td:first').html();
	contentApp.switchMenu("testPaperGenerateList",paperId+";"+paperName);
	return false;
}
/**
 * 编辑试卷
...
function queryForTopic(){
	var topicImportDiv = $('#topicImport>div');
	...
	$('div#'+timestamp).load(file,{'queryStyle':queryStyle,'topicType':topicType,'topicRange':topicRange,'topicName':topicName},function(){
		var div = $('div#'+timestamp+'>div');
		$(content).remove();
		$(div).unwrap();
	});
