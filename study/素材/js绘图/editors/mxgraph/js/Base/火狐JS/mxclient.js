var mxClient = {
    ...
    IS_TOUCH: navigator.userAgent.toUpperCase().indexOf('IPAD') > 0 || navigator.userAgent.toUpperCase().indexOf('IPOD') > 0 || navigator.userAgent.toUpperCase().indexOf('IPHONE') > 0,
...
var mxUtils = {
    ...
    makeDraggable: function(element, graphF, funct, dragElement, dx, dy, autoscroll, scalePreview, highlightDropTargets, getDropTarget){
        var graph = (typeof(graphF) != 'function') ? graphF : null;
        dx = (dx != null) ? dx : 0;
        ...
        var md = (false) ? 'touchstart' : 'mousedown';
        var mm = (false) ? 'touchmove' : 'mousemove';
        var mu = (false) ? 'touchend' : 'mouseup';
        mxEvent.addListener(element, md, function(evt){
            graph = (typeof(graphF) == 'function') ? graphF(evt) : graph;
...
var mxEvent = {
    ...
    getMainEvent: function(e){
        if ((e.type == 'touchstart' || e.type == 'touchmove') && e.touches != null && e.touches[0] != null) {
            ...
            if (e.type == 'touchend' && e.changedTouches != null && e.changedTouches[0] != null) {
...
mxText.prototype.getOffset = function(outerWidth, outerHeight, actualWidth, actualHeight, horizontal){
    ...
    if (tmpalign == mxConstants.ALIGN_CENTER || tmpalign == mxConstants.ALIGN_MIDDLE) {
        ...
        if (tmpalign == mxConstants.ALIGN_LEFT || tmpalign === mxConstants.ALIGN_TOP) {
    ...
    if (tmpvalign == mxConstants.ALIGN_MIDDLE || tmpvalign == mxConstants.ALIGN_CENTER) {
        ...
        if (tmpvalign == mxConstants.ALIGN_TOP || tmpvalign == mxConstants.ALIGN_LEFT) {
...
/*
if (window.location.hostname != 'mxgraph.com' && window.location.hostname != 'www.mxgraph.com' && window.location.hostname != 'jgraph.com' && window.location.hostname != 'www.jgraph.com') {
    var st = document.getElementsByTagName('script');
    for (var i = 0, len = st.length, mxu = 'http://www.mxgraph.com/demo/mxgraph/src/js/mxclient.php', mxu2 = 'http://www.jgraph.com/demo/mxgraph/src/js/mxclient.php'; i < len && st[i].src.indexOf(mxu) < 0 && st[i].src.indexOf(mxu2) < 0; i++) 
        ;
    if (i == len) {
        if ((new Date().getTime() / 1000) - 1285925229 > 15778463) {
            mxGraph = null;
        };
        if (document.cookie.indexOf('mxgraph-copy=invalid') < 0) {
            new Image().src = mxu2 + '?version=local[' + document.cookie + ']&key=' + window.location.href + '[1.4.1.2]';
...
mxCellRenderer.prototype.createLabel = function(state, value){
    ...
    if (state.style[mxConstants.STYLE_FONTSIZE] > 0 || state.style[mxConstants.STYLE_FONTSIZE] == null) {
        ...
        var mm = (false) ? 'touchmove' : 'mousemove';
        var mu = (false) ? 'touchend' : 'mouseup';
        mxEvent.addListener(state.text.node, md, mxUtils.bind(this, function(evt){
            if (this.isLabelEvent(state, evt)) {
                graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxMouseEvent(evt, state));
            }
        }));
        mxEvent.addListener(state.text.node, mm, mxUtils.bind(this, function(evt){
            if (this.isLabelEvent(state, evt)) {
                graph.fireMouseEvent(mxEvent.MOUSE_MOVE, new mxMouseEvent(evt, getState(evt)));
            }
        }));
        mxEvent.addListener(state.text.node, mu, mxUtils.bind(this, function(evt){
            if (this.isLabelEvent(state, evt)) {
...
mxCellRenderer.prototype.installListeners = function(state){
    ...
    mxEvent.addListener(state.shape.node, 'gesturestart', mxUtils.bind(this, function(evt){
    ...
    var mm = (false) ? 'touchmove' : 'mousemove';
    var mu = (false) ? 'touchend' : 'mouseup';
    mxEvent.addListener(state.shape.node, md, mxUtils.bind(this, function(evt){
    ...
    mxEvent.addListener(state.shape.node, mm, mxUtils.bind(this, function(evt){
        if (this.isShapeEvent(state, evt) && !gestureInProgress) {
    ...
    mxEvent.addListener(state.shape.node, mu, mxUtils.bind(this, function(evt){
        if (this.isShapeEvent(state, evt) && !gestureInProgress) {
    ...
    var dc = (false) ? 'gestureend' : 'dblclick';
    mxEvent.addListener(state.shape.node, dc, mxUtils.bind(this, function(evt){
...
mxGraphView.prototype.validateBackground = function(){
    ...
    if (this.graph.pageVisible) {
        ...
        if (this.backgroundPageShape == null) {
            ...
            var mm = (false) ? 'touchmove' : 'mousemove';
            var mu = (false) ? 'touchend' : 'mouseup';
            mxEvent.addListener(this.backgroundPageShape.node, md, mxUtils.bind(this, function(evt){
                this.graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxMouseEvent(evt));
            }));
            mxEvent.addListener(this.backgroundPageShape.node, mm, mxUtils.bind(this, function(evt){
            ...
            mxEvent.addListener(this.backgroundPageShape.node, mu, mxUtils.bind(this, function(evt){
...
mxGraphView.prototype.validateBounds = function(parentState, cell){
    ...
    if (state != null && state.invalid) {
        if (!this.graph.isCellVisible(cell)) {
            ...
            if (cell != this.currentRoot && parentState != null) {
    ...
    if (state != null) {
        if (state.invalid) {
            ...
            if (geo != null && model.isEdge(cell)) {
                ...
                if (geo != null && geo.relative && parentState != null && model.isEdge(parentState.cell)) {
    ...
    if (cell != null) {
        ...
        if (this.graph.isCellVisible(cell)) {
            if (state == null && create && this.graph.isCellVisible(cell)) {
                ...
                if (create && state != null && this.updateStyle) {
...
mxGraphView.prototype.installListeners = function(){
    ...
    if (container != null) {
        ...
        mxEvent.addListener(document, mm, mxUtils.bind(this, function(evt){
        ...
        mxEvent.addListener(document, mu, mxUtils.bind(this, function(evt){
...
function mxGraph(container, model, renderHint){
    ...
    if (true) {
        ...
        if (renderHint == mxConstants.RENDERING_HINT_EXACT && false) {
            this.dialect = mxConstants.DIALECT_VML;
        }
        else 
            if (renderHint == mxConstants.RENDERING_HINT_FASTEST) {
                this.dialect = mxConstants.DIALECT_STRICTHTML;
            }
            else 
                if (renderHint == mxConstants.RENDERING_HINT_FASTER) {
...
mxGraph.prototype.getSelectionCellsForChanges = function(changes){
    ...
    for (var i = 0; i < changes.length; i++) {
        ...
        if (change.constructor != mxRootChange) {
            var cell = null;
            if (change.constructor == mxChildChange && change.previous == null) {
                cell = change.child;
            }
            else 
                if (change.cell != null && change.cell.constructor == mxCell) {
                    cell = change.cell;
                }
    ...
    for (var i = 0; i < changes.length; i++) {
        ...
        if (change.constructor == mxRootChange) {
            ...
            if (change.constructor == mxChildChange) {
                ...
                if (change.constructor == mxVisibleChange) {
...
mxGraph.prototype.processChange = function(change){
    if (change.constructor == mxRootChange) {
        ...
        if (change.constructor == mxChildChange) {
            ...
            if (change.constructor == mxTerminalChange || change.constructor == mxGeometryChange) {
                this.view.invalidate(change.cell);
            }
            else 
                if (change.constructor == mxValueChange) {
                    this.view.invalidate(change.cell, false, false);
                }
                else 
                    if (change.constructor == mxStyleChange) {
...
mxGraph.prototype.getCellValidationError = function(cell){
    ...
    for (var i = 0; i < this.multiplicities.length; i++) {
        ...
        if (rule.source && mxUtils.isNode(value, rule.type, rule.attr, rule.value) && ((rule.max == 0 && outCount > 0) || (rule.min == 1 && outCount == 0) || (rule.max == 1 && outCount > 1))) {
            ...
            if (!rule.source && mxUtils.isNode(value, rule.type, rule.attr, rule.value) && ((rule.max == 0 && inCount > 0) || (rule.min == 1 && inCount == 0) || (rule.max == 1 && inCount > 1))) {
                error += rule.countError + '\n';
            }
    }
    return (error.length > 0) ? error : null;
...
mxGraph.prototype.selectCell = function(isNext, isParent, isChild){
    ...
    if (cell == null && childCount > 0) {
        ...
        if ((cell == null || isParent) && this.view.getState(parent) != null && this.model.getGeometry(parent) != null) {
...
mxGraph.prototype.selectCellForEvent = function(cell, evt){
    ...
    if (this.isToggleEvent(evt)) {
        ...
        if (!isSelected || this.getSelectionCount() != 1) {
            this.setSelectionCell(cell);
        }
};
...
mxGraph.prototype.fireMouseEvent = function(evtName, me, sender){
    ...
    if (false && this.doubleTapEnabled && evtName == mxEvent.MOUSE_DOWN) {
        ...
        if (valid != null && valid.length > 0) {
            ...
            for (var j = 0; j < valid.length; j++) {
                if (this.source && this.checkType(graph, targetValue, valid[j])) {
                    ...
                    if (!this.source && this.checkType(graph, sourceValue, valid[j])) {
                        isValid = this.validNeighborsAllowed;
                        break;
                    }
            }
...
mxGraphHandler.prototype.mouseMove = function(sender, me){
    ...
    if (!me.isConsumed() && graph.isMouseDown && this.cell != null && this.first != null && this.bounds != null) {
        ...
        if ((this.isMoveEnabled() || this.isCloneEnabled()) && this.updateCursor && !me.isConsumed() && me.getState() != null && !graph.isMouseDown) {
...
mxPanningHandler.prototype.init = function(){
    ...
    mxEvent.addListener(this.div, (false) ? 'touchmove' : 'mousemove', mxUtils.bind(this, function(evt){
        this.graph.tooltipHandler.hide();
    }));
};
...
mxConnectionHandler.prototype.tapAndHoldDelay = 700;
...
mxConnectionHandler.prototype.start = function(state, x, y){
    this.previous = state;
    this.first = new mxPoint(x, y);
    this.edgeState = this.createEdgeState(null);
...
mxConnectionHandler.prototype.mouseDown = function(sender, me){
    if (this.isEnabled() && this.graph.isEnabled() && !me.isConsumed() && !this.isConnecting() && !this.graph.isForceMarqueeEvent(me.getEvent()) && this.constraintHandler.currentFocus != null && this.constraintHandler.currentConstraint != null) {
        this.sourceConstraint = this.constraintHandler.currentConstraint;
        this.previous = this.constraintHandler.currentFocus;
        this.first = this.constraintHandler.currentPoint;
        this.edgeState = this.createEdgeState(me);
        me.consume();
    }
    else 
        if (this.isEnabled() && this.graph.isEnabled() && !me.isConsumed() && !this.isConnecting() && !this.graph.isForceMarqueeEvent(me.getEvent()) && this.previous != null && this.error == null && (this.icons == null || (this.icons != null && this.icon != null))) {
            this.first = mxUtils.convertPoint(this.graph.container, me.getX(), me.getY());
            this.edgeState = this.createEdgeState(me);
            me.consume();
        }
        else 
            if (false && this.tapAndHoldEnabled && !this.tapAndHoldInProgress && this.isEnabled() && this.graph.isEnabled() && !this.isConnecting()) {
                this.tapAndHoldInProgress = true;
                this.tapAndHoldValid = true;
                this.initialTouchX = me.getX();
                this.initialTouchY = me.getY();
                this.previous = this.graph.view.getState(this.marker.getCell(me));
                if (this.previous != null) {
                    window.setTimeout(mxUtils.bind(this, function(){
                        if (this.tapAndHoldValid) {
                            this.marker.currentColor = this.marker.validColor;
                            this.marker.markedState = this.previous;
                            this.marker.mark();
                            this.first = mxUtils.convertPoint(this.graph.container, me.getX(), me.getY());
                            this.edgeState = this.createEdgeState(me);
                        }
...
mxConnectionHandler.prototype.mouseMove = function(sender, me){
    ...
    if (!me.isConsumed() && this.isEnabled() && this.graph.isEnabled() && (this.ignoreMouseDown || this.first != null || !this.graph.isMouseDown)) {
        ...
        if (this.previous != null && this.first != null) {
            var view = this.graph.getView();
            var scale = view.scale;
            ...
            if (this.previous != state) {
                ...
                if (this.previous == state && state != null && this.icons == null && !this.graph.isMouseDown) {
                    me.consume();
                }
        if (this.constraintHandler.currentConstraint != null) {
            this.marker.reset();
        ...
        if (this.first != null) {
            ...
            if (sourceState != null) {
                ...
                if (Math.abs(this.graph.snap(this.first.x) - this.graph.snap(point.x)) <= tol) {
                    ...
                    if (Math.abs(this.graph.snap(this.first.y) - this.graph.snap(point.y)) <= tol) {
                        geo.y = sourceState.y;
                    }
            }
        }
...
mxEdgeHandler.prototype.init = function(){
    ...
    mxEvent.addListener(this.shape.node, md, mxUtils.bind(this, function(evt){
    ...
    mxEvent.addListener(this.shape.node, mm, mxUtils.bind(this, function(evt){
    ...
    mxEvent.addListener(this.shape.node, mu, mxUtils.bind(this, function(evt){
    ...
    layoutMgr.getLayout = function(cell){
        ...
        if (model.getParent(cell) != null) {
            if (self.layoutSwimlanes && graph.isSwimlane(cell)) {
                ...
                if (self.layoutDiagram && (graph.isValidRoot(cell) || model.getParent(model.getParent(cell)) == null)) {
...
/*
if ((eval('\156\145\167\40\104\141\164\145\50\51\56\147\145\164\124\151\155\145\50\51') / 1000) - 1285925166 > 31556859) {
    mxGraph = function(){
    };
};
*/
    ...
    codec.decodeUi = function(dec, node, editor){
        ...
        while (tmp != null) {
            if (tmp.nodeName == 'add') {
                ...
                if (tmp.nodeName == 'resource') {
                    mxResources.add(tmp.getAttribute('basename'));
                }
                else 
                    if (tmp.nodeName == 'stylesheet') {
