...
function add_skill(button, content){
  var new_id = new Date().getTime();
  $$('.editing .skills')[0].insert({bottom: content.replace(/new_certificates/g, new_id)});
}
...
document.observe("dom:loaded",function(){
  ...
  document.observe('click', function(event){
    ...
    if(el = event.findElement('#my_resume .control .expand')){
      ...
      event.stop();
    } else if(el = event.findElement('.project a.del')) {
      if(confirm("确定要删除该项目经验吗？")){
        if(/\#$/.test(el['href'])){
          el.up('.project').remove();
        } else {
          new Ajax.Request(el['href'], {
            method: 'delete',
            onSuccess: function(res){
      ...
      event.stop();
    } else if(el = event.findElement('#add_custom_skill')){
      var custom_skill = $('custom_skill')['value'];
      if(!custom_skill.blank()){
        var options = ['a', 'b', 'c'].map(function(i){
          var level_text = {a:'精通', b:'良好', c:'一般'}[i];
          return "<label><input type='radio' name='resume[skills][" + custom_skill + "]' value='" + i + "'/>" + level_text + "　</label>";
        }).join('');
        $$('ul.skills')[0].insert({bottom: "<li><span class='fr'><a href='#' class='del'>删除该技能</a></span><span class='title'>" + custom_skill + "</span><span class='level'>良好度：" + options + "</span></li>"});
      }
  ...
  document.observe('change', function(){
    if(select = event.findElement('#my_resume .skills .title select')){
  ...
  (function(){
    ...
    var Render = {
      ...
      template_page_comment: function(notification) {
        this.render_comments_common_template(notification, 'page', '评论了您的专栏', 'comments');
      },

      template_page_comment_other: function(notification) {
        this.render_comments_common_template(notification, 'page', '评论了您评论过的专栏', 'comments');
      },
