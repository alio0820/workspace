...
//	            map.put("date_y", sf.format(date));//日期
	        ...
	        //控制台打印出天气
	        System.out.println("城市\t日期\t\t星期\t发布时间\t天气\t温度\t风况\t风速\t今天的穿衣指数\t紫外指数\t旅游指数\t舒适指数\t晨练指数\t洗车指数\t穿衣指数");
	       for(int j=0;j<list.size();j++){
	           Map<String,Object> wMap = list.get(j);
	           System.out.println(wMap.get("city")+"\t"+wMap.get("date_y")+"\t"+wMap.get("week")+"\t"+wMap.get("fchh")+"\t"
