...
<HTML>
	<HEAD>
		...
		<STYLE type=text/css>
			...
			.ResultM_1{
	    		background:url(images/districtLeaderInstruction.png);width:362px;height:57px;float:left;
	    	}
		</STYLE>
		<script type='text/javascript'>
			function shenhe(projectId,taskMonth){
				...
				var rate_string = document.getElementsByName("rate_"+name)[0].value;
				var tongguo = document.getElementsByName(selectName)[0].value;			//table中的select无法通过form对象来取得
				alert(tongguo);
				document.form1.action="projectR_duchashenhe.action?projectId="+projectId+"&taskMonth="+taskMonth+"&tongguo="+tongguo+"&yijian="+yijian+"&rate_string="+rate_string;
					...
					<TR>
						...
						<TD
							...
							<div style="margin-left:30px;margin-top:20px;">
								<table class='xiangmu' width='1300px' cellSpacing=0 cellPadding=4>
									...
									<%
										...
										for(QianTouViewBean prb:list ){
												...
												out.println("<tr><td width='200px' class='borderleft'>"+prb.getProjectName()+"</td>");	//项目名
												out.println("<td width='100px'>"+prb.getResponsibleDepartemntName()+"</td>");			//责任部门
												out.println("<td width='50px'>"+prb.getTaskMonth()+"</td>");							//任务月份
												out.println("<td width='400px'>"+prb.getActualFinish()+"</td>");						//该月进展情况
												out.println("<td width='100px'>"+prb.getRate()+"<input type='hidden' name='rate_"+name+"'value='"+prb.getRate()+"'/></td>");								//该月进度
												out.println("<td width='150px'>"+prb.getLeadCompanyView()+"</td>");						//牵头部门批示
												out.println("<td width='150px'>"+prb.getSeparateLeaderView()+"</td>");					//区分管领导批示
												out.println("<td width='70px'><select name='select_"+name+"'><option value='T' >通过</option><option value='F' selected>不通过</option></select></td>");//督查室审批
												out.println("<td width='150px'><textarea name='text_"+name+"'></textarea></td>");
												out.println("<td width='60px'><input type='button' value='提交' onclick='shenhe("+projectId+","+taskMonth+")'/></td></tr>");//督查室批示
