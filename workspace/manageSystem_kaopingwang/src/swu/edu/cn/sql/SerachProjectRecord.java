...
//		ArrayList<ProjectRecordBean> list=spr.searchProjectByNoReport(112, 1);
		ArrayList<ProjectRecordBean> list=spr.searchProjectByReportFaild(112);
		 for(ProjectRecordBean prb : list){
				...
				System.out.print(prb.getTaskMonth()+"月\t");
				...
				System.out.print(prb.getLeadCompanyView());
	...
	public ArrayList<ProjectRecordBean>  searchProjectByReportFaild(int departmentID,int taskMonth){
		...
		String selectSQL=" select  Project.projectID, projectName,leader,firstClassify,secondClassify,ProjectClassify.thirdClassify ,Project.expectTask,Project.comment,Department.departmentName,currentProgress,ProjectRecord.taskMonth,status,leadCompanyView,separateLeaderView,supervisionView  " +
	 ...
	 * 返回该部门所有提交没通过的项目记录
	 */
	public ArrayList<ProjectRecordBean>  searchProjectByReportFaild(int departmentID){
		ArrayList<ProjectRecordBean> list=new ArrayList<ProjectRecordBean>();
		int len=(new java.util.Date()).getMonth()+1;
		for(int i=len;i>=1;i--){
			ArrayList<ProjectRecordBean> list1=searchProjectByReportFaild(departmentID,i);
				for(ProjectRecordBean prb:list1)
						list.add(prb);
...
//					System.out.println(rs.getInt("ProjectRecord.taskMonth"));
	...
	public ArrayList<ProjectRecordBean>  searchProjectRecordByDetails(int projectID){
		...
		String selectSQL="select projectName,leader,rate,expectTask,ProjectRecord.taskMonth,ProjectRecord.actualFinish,status,Project.comment,Project.currentProgress,Department.departmentName,ProjectClassify.firstClassify, ProjectClassify.secondClassify,ProjectClassify.thirdClassify,leadCompanyView,separateLeaderView,supervisionView  " +
		...
		try {
			while(rs.next()){
					...
					prb.setProjectName(rs.getString("projectName"));
					prb.setStatus(rs.getString("status"));
					prb.setActualFinish(rs.getString("ProjectRecord.actualFinish"));
