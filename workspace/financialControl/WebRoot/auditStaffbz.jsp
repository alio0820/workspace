		...
		<script type='text/javascript'>
			function check(link){
				...
				if(document.getElementsByName(Apply_ID)[0].checked)		//radio选中是属性
					...
					isFinCheck='false';
				document.form1.action="finCheck_FinCheckUpdate.action?Apply_ID="+Apply_ID+"&isFinCheck="+isFinCheck;
					...
					<TR>
						...
						<TD
							...
							<form name='form1' method ='post' action ='appForm_insertForm.action'>
								...
								<%
									FinanicalCheck finchCheck=new FinanicalCheck(); //实例化一个财务人员检查实例
									finchCheck.OpenDB();
									
									ArrayList<ApplicationForm> list=finchCheck.FinCheck(); //调用finCheck（）方法，读取数据库审核表
									...
									for(ApplicationForm af:list){
										...
										out.print("<td >"+af.getReceiptAccount()+"</td>");
