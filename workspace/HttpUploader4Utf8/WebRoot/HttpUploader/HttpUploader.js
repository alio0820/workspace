...
function HttpUploaderMgr()
{
	...
	this.Init = function()
	{
		...
		this.Partition = document.getElementById("objHttpUploaderPartition");
		...
		//粘贴文件
		obj = document.getElementById("btnPasteFile");
		if (obj) obj.attachEvent("onclick", function() { mgr.PasteFiles(); });
		//清空已完成文件
...
function HttpUploader(fileID, filePath, mgr)
{
	...
	this.ATL.RangeSize = this.Config["RangeSize"];
...
function HttpUploader_Error(obj)
{
	...
	//文件大小超过限制,文件大小为0
	if (10 == obj.ATL.ErrorCode || 201 == obj.ATL.ErrorCode)
	{
...
function HttpUploader_Process(obj, speed, postedLength, percent, time)
{
	...
	$.ajax({
		...
		, data: { uid: obj.Fields["uid"], fid: obj.fid, PostedLength: obj.ATL.PostedLength, Percent: percent, time: Date() }
