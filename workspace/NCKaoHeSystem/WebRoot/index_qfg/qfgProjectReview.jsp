...
<style type="text/css">
		...
		<script type='text/javascript'>
			function shenhe(projectId,taskMonth){
				...
				alert(tongguo);
				document.form1.action="projectR_qufenguan.action?projectId="+projectId+"&taskMonth="+taskMonth+"&tongguo="+tongguo+"&yijian="+yijian;
...
<div class="zhong" >
  ...
  <div class="menu" >
    ...
    <div id="menu3" class="menusel">
      <h2><a href="qfgProjectReview.jsp">项目管理</a></h2>
      <div class="position">
        <ul class="clearfix typeul">
          <li><a href="qfgProjectReview.jsp">区领导审核</a></li>
		  <li class="lli"><a href="qfgProjectInquires.jsp">搜索</a></li>
        </ul>
      </div>
    </div>    
  ...
  <div style="color:black;height:auto;">
  	<div id="dutyProject_top" style="height:20px;margin-bottom:10px">
        ...
        <div style="float:left;margin-left:100px"><span style="font-weight:bold">当前位置：</span>
                ...
                <a href="qfgProjectReview.jsp" style="color:rgb(120,48,120);">区领导审核</a>
        </div>
    </div>
    	...
    	<div style='margin:5px 0px 5px 3px;'>
       	...
       	<table class='xiangmu' width='1365px' style='margin-left:-200px;background-color:white;' cellSpacing=0 cellPadding=4>
				...
				<%
					...
					ArrayList<ProjectMonthRecordBean> list=pm.QianTouLeaderViewList(leadarName);
					for(ProjectMonthRecordBean pmrb:list ){
							...
							out.println("<td ><textarea  style ='width:190px;height:40px;font-size:12px' name='text_"+name+"'></textarea></td>");%>
							<td width='60px' style='text-align:center'><input type='button' value='提交' onclick="shenhe('<%=projectId %>','<%=taskMonth %>')"/></td></tr>														
					<%}%>
			</table>
			<div style='margin:5px 0px 5px 3px;'>
	       		2.非项目指标审核：
			...
			<%
				ArrayList<NoProjectEvaluationBean> list1= pm.QianTouLeaderNoProjectViewList(leadarName);
				if(list.size()==0)
				...
				for(NoProjectEvaluationBean  npeb:list1){
					...
					out.println("<td width='60px'><span>"+npeb.getStatus().charAt(0)+"</span></td>");
