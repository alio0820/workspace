...
public class BorrowDAO extends BaseHibernateDAO {
	private static final Log log = LogFactory.getLog(BorrowDAO.class);
	...
	public Borrow findById(com.BorrowId id) {
		log.debug("getting Borrow instance with id: " + id);
		try {
			Borrow instance = (Borrow) getSession().get("com.Borrow", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Borrow instance) {
		...
		try {
			List results = getSession().createCriteria("com.Borrow").add(
