...
import com.zr.js.js.service.PropertyService;
import com.zr.js.js.service.impl.PropertyServiceImpl;
...
//		System.out.println("会计名称:" +name);
		...
		boolean flag = propertyService.property(str,num, name,qq,id,id_forward);
		if(flag==false){
		//addActionError("资产类别编号已存在，请重新输入！");
		}
		propertyService = new PropertyServiceImpl();
		intRowCount=propertyService.count(str);
		...
		this.setList(propertyService.queryUser(i,pagesize,str,id_forward));
		if(str.equals("1"))
			return "next";
		else
		    return SUCCESS;
