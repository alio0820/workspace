...
public  UserBean  isUserExistAndLegal(String userId,String password){
	...
	UserBean ub=null;
	try {
		ps=conn.prepareStatement("select * from user where valid =1 and userId=? and password=?");
		...
		if(rs.next()){
			...
			ub.setLastTime(rs.getString("lastTime"));
			ub.setLastIp(rs.getString("lastIp"));
			ub.setUpdateTime(rs.getString("updateTime"));
		}
		else{
			return null;
		}
		ps=conn.prepareStatement("select * from mail where userId=?");
		ps.setString(1, userId);
		rs=ps.executeQuery();
		String[] mail = null;
		int i=0;
		while(rs.next()){
			mail[i++] = rs.getString("emailUserName")+";"+rs.getString("emailPassword");
		}
		ub.setEmailAddress(mail);
		ps.close();
		rs.close();
		return  ub;
	} catch (SQLException e) {
